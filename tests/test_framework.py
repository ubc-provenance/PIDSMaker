...
def prepare_cfg(
    ...
    if featurization:
        args.__dict__["feat_training.used_method"] = featurization
    if encoder:
        args.__dict__["gnn_training.encoder.used_methods"] = encoder
    if objective:
        args.__dict__["gnn_training.decoder.used_methods"] = objective
    if decoder and objective:
        args.__dict__[f"gnn_training.decoder.{objective}.decoder"] = decoder

    if encoder and "tgn" not in encoder:
        args.__dict__["graph_preprocessing.intra_graph_batching.used_methods"] = "edges"
...
class TestBatching:
    ...
    def test_global_batching(self, dataset, device, global_batching_method):
        custom_args = [
            ("graph_preprocessing.global_batching.used_method", global_batching_method),
            ("graph_preprocessing.global_batching.global_batching_batch_size", 1000),
        ]
        bs = None
        ...
        if bs:
            custom_args.append(
                ("graph_preprocessing.global_batching.global_batching_batch_size", bs)
            )
    ...
    def test_intra_graph_batching(self, dataset, device, intra_graph_batching_method):
        custom_args = [
            (
                "graph_preprocessing.intra_graph_batching.used_methods",
                intra_graph_batching_method,
            ),
            (
                "graph_preprocessing.intra_graph_batching.edges.intra_graph_batch_size",
                200,
            ),
        ]
        if "tgn_last_neighbor" not in intra_graph_batching_method:
            custom_args.append(("gnn_training.encoder.used_methods", "graph_attention"))
    ...
    def test_inter_graph_batching(self, dataset, device, inter_graph_batching_method):
        custom_args = [
            (
                "graph_preprocessing.inter_graph_batching.used_method",
                inter_graph_batching_method,
            ),
            ("graph_preprocessing.inter_graph_batching.inter_graph_batch_size", 2),
            ("graph_preprocessing.intra_graph_batching.used_methods", "none"),
            ("gnn_training.encoder.used_methods", "graph_attention"),
        ]
