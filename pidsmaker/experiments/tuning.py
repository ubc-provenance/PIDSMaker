...
def fuse_cfg_with_sweep_cfg(cfg, sweep_cfg):
    ...
    for key, value in sweep_cfg.items():
        ...
        if key == "embedding_techniques":
            if value == "no_featurization":
                ...
                cfg.feat_training.training_split = split

                # If a model doesn't use embedding in features, we add them to benchmark
                if "node_emb" not in cfg.graph_preprocessing.node_features:
                    cfg.graph_preprocessing.node_features += ",node_emb"

        elif key == "orthrus_node_label_features":
            if value == True:
                cfg.build_graphs.node_label_features.subject = "type, path, cmd_line"
                cfg.build_graphs.node_label_features.file = "type, path"
                cfg.build_graphs.node_label_features.netflow = (
    ...
    # Special cases
    if cfg.gnn_training.node_out_dim == -1:
        cfg.gnn_training.node_out_dim = cfg.gnn_training.node_hid_dim
    elif cfg.gnn_training.node_out_dim == -2:
        cfg.gnn_training.node_out_dim = cfg.gnn_training.node_hid_dim // 2

    if cfg.gnn_training.encoder.tgn.tgn_memory_dim == -1:
        cfg.gnn_training.encoder.tgn.tgn_memory_dim = (
            cfg.gnn_training.node_hid_dim
        )
    if cfg.gnn_training.encoder.tgn.tgn_time_dim == -1:
        cfg.gnn_training.encoder.tgn.tgn_time_dim = (
            cfg.gnn_training.node_hid_dim
        )
