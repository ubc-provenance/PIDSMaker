...
def get_msg2vec(indexid2msg, model_path):
    ...
    log(
        f"Number of subjects: {num_sub}; Number of files: {num_file}; Number of netflows: {num_net}"
    )

    log("Finish generating normalized node vectors.")
...
def get_attrs_different_files(indexid2msg, model_path):
    ...
    embeddings["/data/*"] = {"embeddings": file_list_1, "color": "r"}
    ...
    embeddings["/proc/*"] = {"embeddings": file_list_2, "color": "g"}
    ...
    embeddings["/home/admin/*"] = {"embeddings": file_list_3, "color": "b"}
    ...
    embeddings["/usr/share/*"] = {"embeddings": file_list_4, "color": "yellow"}
...
def get_attrs_one_type(msg2vec, num_each_type, sampled_type):
    ...
    embeddings[type_name] = {"embeddings": [embeddings_sub[i] for i in sampled_sub], "color": color}

    log("Finish generating node embeddings.")
...
def get_attrs_each_type(msg2vec, num_each_type):
    ...
    embeddings = {}
    sampled_sub = random.sample(range(len(embeddings_sub)), num_each_type)
    embeddings["subject"] = {"embeddings": [embeddings_sub[i] for i in sampled_sub], "color": "r"}

    sampled_file = random.sample(range(len(embeddings_file)), num_each_type)
    embeddings["file"] = {"embeddings": [embeddings_file[i] for i in sampled_file], "color": "g"}

    sampled_net = random.sample(range(len(embeddings_net)), num_each_type)
    embeddings["netflow"] = {"embeddings": [embeddings_net[i] for i in sampled_net], "color": "b"}
...
def build_tsne_visualization(embeddings, fig_path):
    ...
    for key, value in embeddings.items():
        ...
        dic_2d[key] = {"embeddings_2d": embeddings_2d, "color": value["color"]}

    log("Start visualizing TSNE embeddings")
    plt.figure(figsize=(10, 8))

    for key, value in dic_2d.items():
        plt.scatter(
            value["embeddings_2d"][:, 0],
            value["embeddings_2d"][:, 1],
            marker=".",
            color=value["color"],
            label=key,
        )
