...
from provnet_utils import compute_class_weights, log
...
class FewShotEdgeDetection(nn.Module):
    ...
    def forward(self, h_src, h_dst, y_edge, inference, **kwargs):
        ...
        return {"loss": self.loss_fn(y_edge_hat, y_edge, inference=inference)} #weight=torch.tensor([1.0, 20000.0]).to(h_src.device))
...
class ValidationContrastiveStopper(nn.Module):
    ...
    def forward(self, edge_type, y_edge, validation, inference, *args, **kwargs):
        results = self.objective(*args, edge_type=edge_type, y_edge=y_edge, inference=inference, **kwargs)
...
class EdgeContrastiveDecoder(nn.Module):
    ...
    def _fast_negative_sampling(self, edge_index, h_src, h_dst, aug_coef=1.0):
        (h_src, h_dst), edge_index, _ = self.graph_reindexer._reindex_graph(edge_index, h_src, h_dst, x_is_tuple=True)
