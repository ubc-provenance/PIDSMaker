...
def compute_hetero_features(g, node_map, edge_map):
    return _compute_hetero_features(
        g.edge_index, g.x, g.node_type_argmax, g.edge_type_argmax, node_map, edge_map
    )
...
def _compute_hetero_features(edge_index, x, node_type_argmax, edge_type_argmax, node_map, edge_map):
    ...
    for triplet in unique_triplets:
        ...
        src_local_indices = torch.searchsorted(
            node_type_to_indices[src_t_str], hetero_edge_index[0]
        )
        dst_local_indices = torch.searchsorted(
            node_type_to_indices[dst_t_str], hetero_edge_index[1]
        )
