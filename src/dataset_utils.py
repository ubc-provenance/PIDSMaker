...
def decrement_dict(d):
    return {k-1 if isinstance(k, int) else k: v-1 if isinstance(v, int) else v for k, v in d.items()}
...
def get_num_edge_type(cfg):
     if cfg.dataset.name not in OPTC_DATASETS and "edge_type_triplet" in cfg.detection.graph_preprocessing.edge_features:
...
def get_rel2id_considering_triplets(cfg):
     if "edge_type_triplet" in cfg.detection.graph_preprocessing.edge_features:
          return {i+1: e for i, e in enumerate([event for events in possible_events.values() for event in events])}
