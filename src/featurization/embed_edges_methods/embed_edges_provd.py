...
from config import *
from featurization.embed_nodes_methods.provd_embed_paths import node2corpus
...
def main(cfg):
    ...
    node2corpus = get_node2corpus(splits=["train", "val", "test"], cfg=cfg)
    ...
    path_emb2nodes = defaultdict(set)
    path2vector_cache = {}
    ...
    for indexid, _ in tqdm(indexid2msg.items(), desc='Embeding all paths in the dataset'):
        ...
        for path in corpus:
            if path not in path2vector_cache:
                model.random.seed(0)
                vector = model.infer_vector(path)
                path2vector_cache[path] = vector
                
            vector = path2vector_cache[path]
            path_emb2nodes[vector].add(indexid)
